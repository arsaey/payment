...
class Nowpayments
{
    ...
    {
        $this->env = Config::get('payments.nowpayments.env');
    }
    ...
    {
        if ($this->env == "sandbox")
            $this->baseUrl = Config::get('payments.nowpayments.sandboxUrl');
        else
            $this->baseUrl = Config::get('payments.nowpayments.liveUrl');
        $this->callbackUrl = Config::get('payments.nowpayments.callbackUrl');
    }
    ...
    {
        $this->apiKey = Config::get('payments.nowpayments.apiKey');
    }
    ...
    public function verifyIPN(): bool
    {
        ...
        if ($receivedHmac) {
            ...
            if ($requestData->isNotEmpty()) {
                $hmac = hash_hmac("sha512", $sortedRequestJson, trim(config('payments.nowpayments.ipnSecret')));
